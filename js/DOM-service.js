...
function display(elSelector, value) {
    document.querySelector(elSelector).style.display = value
}
...
function getKeyWordclass(keyWordValue) {
    if (keyWordValue < 0) return 'very-few'
    else if (keyWordValue < 1) return 'few'
    else if (keyWordValue < 2) return 'avg'
    else if (keyWordValue < 3) return 'much'
    return 'very-much'
}
